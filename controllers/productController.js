import { catchAsyncErrors } from "../middlewares/catchAsyncError.js";
import ErrorHandler from "../middlewares/errorMiddleware.js";
import { v2 as cloudinary } from "cloudinary";
import database from "../database/db.js";
import { getAIRecommendation } from "../utils/getAIRecommendation.js";
const addSmartTextFilter = (value, conditions, values, indexRef) => {
  conditions.push(`
    (
      name ILIKE $${indexRef.i}
      OR description ILIKE $${indexRef.i}
      OR category ILIKE $${indexRef.i}
    )
  `);
  values.push(`%${value}%`);
  indexRef.i++;
};

const toSlug = (text) =>
  text
    .toLowerCase()
    .replace(/&/g, "and")
    .replace(/\s+/g, "-")
    .trim();
    // 🔁 AI fail ho jaye to keyword fallback
const fallbackKeywordParser = (text) => {
  const lower = text.toLowerCase();

 const productKeywords = {
  "gpu": ["gpu", "graphics card", "graphic card", "rtx", "gtx", "radeon"],
  "cpu": ["cpu", "processor", "ryzen", "intel"],
  "t-shirt": ["tshirt", "t-shirt", "tee", "t shirt"],
  "shirt": ["shirt"],
  "jacket": ["jacket", "coat"],
  "shoe": ["shoe", "shoes", "sneaker"],
};


  for (const [productType, keywords] of Object.entries(productKeywords)) {
    if (keywords.some((k) => lower.includes(k))) {
   return {
  productType,
  category: ["gpu", "cpu"].includes(productType)
    ? "electronics"
    : "fashion",
  color: null,
  material: null,
  gender: null,
  usage: null,
  minPrice: null,
  maxPrice: null,
};

    }
  }


};



export const createProduct = catchAsyncErrors(async (req, res, next) => {
  const { name, description, price, category, stock } = req.body;
  const created_by = req.user.id;

  if (!name || !description || !price || !category || !stock) {
    return next(
      new ErrorHandler("Please provide complete product details.", 400)
    );
  }

  let uploadedImages = [];
  if (req.files && req.files.images) {
    const images = Array.isArray(req.files.images)
      ? req.files.images
      : [req.files.images];

    for (const image of images) {
      const result = await cloudinary.uploader.upload(image.tempFilePath, {
        folder: "Ecommerce_Product_Images",
        width: 1000,
        crop: "scale",
      });

      uploadedImages.push({
        url: result.secure_url,
        public_id: result.public_id,
      });
    }
  }

  const product = await database.query(
    `INSERT INTO products (name, description, price, category, stock, images, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
    [
      name,
      description,
      Number(price),

       toSlug(category), 
      stock,
      JSON.stringify(uploadedImages),
      created_by,
    ]
  );

  res.status(201).json({
    success: true,
    message: "Product created successfully.",
    product: product.rows[0],
  });
});

export const fetchAllProducts = catchAsyncErrors(async (req, res, next) => {
  const { availability, price, category, ratings, search } = req.query;
  const page = parseInt(req.query.page) || 1;
  const limit = 10;
  const offset = (page - 1) * limit;

  const conditions = [];
  let values = [];
  let index = 1;

  let paginationPlaceholders = {};

  // Filter products by availability
  // ✅ AVAILABILITY FILTER (FIXED)
if (availability === "instock") {
  conditions.push(`stock > 5`);
}

if (availability === "limited") {
  conditions.push(`stock > 0 AND stock <= 5`);
}

if (availability === "out") {
  conditions.push(`stock = 0`);
}

  // Filter products by price
  if (price) {
    const [minPrice, maxPrice] = price.split("-");
    if (minPrice && maxPrice) {
      conditions.push(`price BETWEEN $${index} AND $${index + 1}`);
      values.push(minPrice, maxPrice);
      index += 2;
    }
  }

  // Filter products by category
  if (category) {
  conditions.push(`LOWER(p.category) = $${index}`);
  values.push(category.toLowerCase());
  index++;
}



  // Filter products by rating
  if (ratings) {
    conditions.push(`ROUND(p.ratings) = $${index}`);
    values.push(Number(ratings));  
    index++;
  }

  // Add search query
  if (search) {
    conditions.push(
      `(p.name ILIKE $${index} OR p.description ILIKE $${index})`
    );
    values.push(`%${search}%`);
    index++;
  }

  const whereClause = conditions.length
    ? `WHERE ${conditions.join(" AND ")}`
    : "";

  // Get count of filtered products
  const totalProductsResult = await database.query(
    `SELECT COUNT(*) FROM products p ${whereClause}`,
    values
  );

  const totalProducts = parseInt(totalProductsResult.rows[0].count);

  paginationPlaceholders.limit = `$${index}`;
  values.push(limit);
  index++;

  paginationPlaceholders.offset = `$${index}`;
  values.push(offset);
  index++;

  // FETCH WITH REVIEWS
  const query = `
    SELECT p.*, 
    COUNT(r.id) AS review_count 
    FROM products p 
    LEFT JOIN reviews r ON p.id = r.product_id
    ${whereClause}
    GROUP BY p.id
    ORDER BY p.created_at DESC
    LIMIT ${paginationPlaceholders.limit}
    OFFSET ${paginationPlaceholders.offset}
    `;

  const result = await database.query(query, values);

  // QUERY FOR FETCHING NEW PRODUCTS
  const newProductsQuery = `
    SELECT p.*,
    COUNT(r.id) AS review_count
    FROM products p
    LEFT JOIN reviews r ON p.id = r.product_id
    WHERE p.created_at >= NOW() - INTERVAL '30 days'
    GROUP BY p.id
    ORDER BY p.created_at DESC
    LIMIT 8
  `;
  const newProductsResult = await database.query(newProductsQuery);

  // QUERY FOR FETCHING TOP RATING PRODUCTS (rating >= 4.5)
  const topRatedQuery = `
    SELECT p.*,
    COUNT(r.id) AS review_count
    FROM products p
    LEFT JOIN reviews r ON p.id = r.product_id
    WHERE p.ratings >= 4
    GROUP BY p.id
    ORDER BY p.ratings DESC, p.created_at DESC
    LIMIT 8
  `;
  const topRatedResult = await database.query(topRatedQuery);

  res.status(200).json({
    success: true,
    products: result.rows,
    totalProducts,
    newProducts: newProductsResult.rows,
    topRatedProducts: topRatedResult.rows,
  });
});

export const updateProduct = catchAsyncErrors(async (req, res, next) => {
  const { productId } = req.params;
  const { name, description, price, category, stock } = req.body;

  const numericPrice = Number(price);

if (
  !name ||
  !description ||
  price === undefined ||
  price === null ||
  isNaN(numericPrice) ||
  !category ||
  stock === undefined ||
  stock === null
) {

    return next(
      new ErrorHandler("Please provide complete product details.", 400)
    );
  }
  const product = await database.query("SELECT * FROM products WHERE id = $1", [
    productId,
  ]);
  if (product.rows.length === 0) {
    return next(new ErrorHandler("Product not found.", 404));
  }
  const result = await database.query(
    `UPDATE products SET name = $1, description = $2, price = $3, category = $4, stock = $5 WHERE id = $6 RETURNING *`,
    [
  name,
  description,
  Number(price),
  toSlug(category),   // ✅ FIX
  stock,
  productId
]


  );
  res.status(200).json({
    success: true,
    message: "Product updated successfully.",
    updatedProduct: result.rows[0],
  });
});

export const deleteProduct = catchAsyncErrors(async (req, res, next) => {
  const { productId } = req.params;

  const product = await database.query("SELECT * FROM products WHERE id = $1", [
    productId,
  ]);
  if (product.rows.length === 0) {
    return next(new ErrorHandler("Product not found.", 404));
  }

  const images = product.rows[0].images;

  const deleteResult = await database.query(
    "DELETE FROM products WHERE id = $1 RETURNING *",
    [productId]
  );

  if (deleteResult.rows.length === 0) {
    return next(new ErrorHandler("Failed to delete product.", 500));
  }

  // Delete images from Cloudinary
  if (images && images.length > 0) {
    for (const image of images) {
      await cloudinary.uploader.destroy(image.public_id);
    }
  }

  res.status(200).json({
    success: true,
    message: "Product deleted successfully.",
  });
});

export const fetchSingleProduct = catchAsyncErrors(async (req, res, next) => {
  const { productId } = req.params;

  const result = await database.query(
    `
        SELECT p.*,
        COALESCE(
        json_agg(
        json_build_object(
            'review_id', r.id,
            'rating', r.rating,
            'comment', r.comment,
            'reviewer', json_build_object(
            'id', u.id,
            'name', u.name,
            'avatar', u.avatar
            )) 
        ) FILTER (WHERE r.id IS NOT NULL), '[]') AS reviews
         FROM products p
         LEFT JOIN reviews r ON p.id = r.product_id
         LEFT JOIN users u ON r.user_id = u.id
         WHERE p.id  = $1
         GROUP BY p.id`,
    [productId]
  );

  res.status(200).json({
    success: true,
    message: "Product fetched successfully.",
    product: result.rows[0],
  });
});


export const postProductReview = catchAsyncErrors(async (req, res, next) => {
  const { productId } = req.params;
  const { rating, comment } = req.body;
  if (!rating || !comment) {
    return next(new ErrorHandler("Please provide rating and comment.", 400));
  }
  const purchasheCheckQuery = `
    SELECT oi.product_id
    FROM order_items oi
    JOIN orders o ON o.id = oi.order_id
    JOIN payments p ON p.order_id = o.id
    WHERE o.buyer_id = $1
    AND oi.product_id = $2
    AND LOWER(p.payment_status) IN ('paid', 'pending')

    LIMIT 1 
  `;

  const { rows } = await database.query(purchasheCheckQuery, [
    req.user.id,
    productId,
  ]);

  if (rows.length === 0) {
    return res.status(403).json({
      success: false,
      message: "You can only review a product you've purchased.",
    });
  }

  const product = await database.query("SELECT * FROM products WHERE id = $1", [
    productId,
  ]);
  if (product.rows.length === 0) {
    return next(new ErrorHandler("Product not found.", 404));
  }

  const isAlreadyReviewed = await database.query(
    `
    SELECT * FROM reviews WHERE product_id = $1 AND user_id = $2
    `,
    [productId, req.user.id]
  );

  let review;

  if (isAlreadyReviewed.rows.length > 0) {
    review = await database.query(
      "UPDATE reviews SET rating = $1, comment = $2 WHERE product_id = $3 AND user_id = $4 RETURNING *",
      [rating, comment, productId, req.user.id]
    );
  } else {
    review = await database.query(
      "INSERT INTO reviews (product_id, user_id, rating, comment) VALUES ($1, $2, $3, $4) RETURNING *",
      [productId, req.user.id, rating, comment]
    );
  }

  const allReviews = await database.query(
    `SELECT AVG(rating) AS avg_rating FROM reviews WHERE product_id = $1`,
    [productId]
  );

  const newAvgRating = allReviews.rows[0].avg_rating;

  const updatedProduct = await database.query(
    `
        UPDATE products SET ratings = $1 WHERE id = $2 RETURNING *
        `,
    [newAvgRating, productId]
  );

  res.status(200).json({
    success: true,
    message: "Review posted.",
    review: review.rows[0],
    product: updatedProduct.rows[0],
  });
});

export const deleteReview = catchAsyncErrors(async (req, res, next) => {
  const { productId } = req.params;
  const review = await database.query(
    "DELETE FROM reviews WHERE product_id = $1 AND user_id = $2 RETURNING *",
    [productId, req.user.id]
  );

  if (review.rows.length === 0) {
    return next(new ErrorHandler("Review not found.", 404));
  }

  const allReviews = await database.query(
    `SELECT AVG(rating) AS avg_rating FROM reviews WHERE product_id = $1`,
    [productId]
  );

  const newAvgRating = allReviews.rows[0].avg_rating;

  const updatedProduct = await database.query(
    `
        UPDATE products SET ratings = $1 WHERE id = $2 RETURNING *
        `,
    [newAvgRating, productId]
  );

  res.status(200).json({
    success: true,
    message: "Your review has been deleted.",
    review: review.rows[0],
    product: updatedProduct.rows[0],
  });
});

export const fetchAIFilteredProducts = catchAsyncErrors(
  async (req, res, next) => {
    const { userPrompt } = req.body;

    if (!userPrompt) {
      return next(new ErrorHandler("Search query required", 400));
    }

    // 🧠 STEP 1: AI understands the query
    let aiFilters = await getAIRecommendation(userPrompt);

if (!aiFilters || Object.keys(aiFilters).length === 0) {
  aiFilters = fallbackKeywordParser(userPrompt);
}


const productTypeMap = {
  "t-shirt": ["t-shirt", "tshirt", "t shirt", "tee"],
  "shirt": ["shirt"],
  "jacket": ["jacket", "coat"],
  "shoe": ["shoe", "sneaker"],
};


if (aiFilters.category) {
  aiFilters.category = toSlug(aiFilters.category);
}


    let conditions = [];
    let values = [];
    let index = 1;

    // 🧩 STEP 2: Dynamic SQL (NO hard coding)
if (aiFilters.productType) {
  const keywords =
    productTypeMap[aiFilters.productType] || [aiFilters.productType];

  const orConditions = keywords.map(
    () => `name ILIKE $${index++}`
  );

  conditions.push(`(${orConditions.join(" OR ")})`);

  keywords.forEach((k) => values.push(`%${k}%`));
}


    if (aiFilters.category) {
      conditions.push(`category ILIKE $${index}`);
      values.push(`%${aiFilters.category}%`);
      index++;
    }

    if (aiFilters.color) {
      conditions.push(`(name ILIKE $${index} OR description ILIKE $${index})`);
      values.push(`%${aiFilters.color}%`);
      index++;
    }

  let indexRef = { i: index };

if (aiFilters.material) {
  addSmartTextFilter(aiFilters.material, conditions, values, indexRef);
}

if (aiFilters.gender) {
  addSmartTextFilter(aiFilters.gender, conditions, values, indexRef);
}

if (aiFilters.usage) {
  addSmartTextFilter(aiFilters.usage, conditions, values, indexRef);
}

index = indexRef.i;


    

    if (aiFilters.minPrice) {
      conditions.push(`price >= $${index}`);
      values.push(aiFilters.minPrice);
      index++;
    }

    if (aiFilters.maxPrice) {
      conditions.push(`price <= $${index}`);
      values.push(aiFilters.maxPrice);
      index++;
    }

    const whereClause = conditions.length
      ? `WHERE ${conditions.join(" AND ")}`
      : "";

      if (conditions.length === 0) {
  return res.status(200).json({
    success: true,
    aiUnderstanding: aiFilters,
    products: [],
    message: "No matching products found"
  });
}

    // 🗄️ STEP 3: Database filtering
let result = await database.query(
  `SELECT * FROM products ${whereClause} ORDER BY created_at DESC`,
  values
);

// 🔁 FINAL FALLBACK: category hata ke try karo
if (result.rows.length === 0 && aiFilters.category) {
  const fallbackConditions = [];
  const fallbackValues = [];
  let fallbackIndex = 1;

  // rebuild conditions EXCEPT category
  conditions.forEach((cond, i) => {
    if (!cond.includes("category")) {
      fallbackConditions.push(
        cond.replace(/\$\d+/g, () => `$${fallbackIndex++}`)
      );
      fallbackValues.push(values[i]);
    }
  });

  const fallbackWhere =
    fallbackConditions.length > 0
      ? `WHERE ${fallbackConditions.join(" AND ")}`
      : "";

  result = await database.query(
    `SELECT * FROM products ${fallbackWhere} ORDER BY created_at DESC`,
    fallbackValues
  );
}


    return res.status(200).json({
      success: true,
      aiUnderstanding: aiFilters,
      products: result.rows,
    });
  }
);
